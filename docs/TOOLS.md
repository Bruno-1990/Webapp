...
- A aba **`Conferencia`** lista **so o que exige acao neste periodo**: numeros sem nota e duplicatas, uma linha cada. Canceladas, denegadas e notas de mes anterior nao entram — ocupam o numero ou pertencem a sequencia de outro mes, e ficam detalhadas na aba da propria serie (amarelo e cinza). As contagens gerais aparecem na tela, no resultado da conferencia.
